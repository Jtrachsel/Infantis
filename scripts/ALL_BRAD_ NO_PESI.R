...
# same thing but with all brad strains

brad_strains <- read_tsv('metadata/Brad_strains.tsv')
infmeta <- read_tsv('output/08_metadata.tsv')



infmeta %>% 
  filter(strain %in% brad_strains$Strain) %>%
  pull(PDS_acc) %>% 
  unique()
...
pesi_refs <- infmeta %>% filter(strain %in% brad_strains$Strain)
...
LOOK <- 
  ...
  write_tsv('all_brad_no_pESI_comp.tsv', col_names = FALSE)

# ran the ppanggolin script

use_directory('all_brad_no_pESI_comp/')

tmp2 <- 
  tmp %>% 
  select(asm_acc, strain, fasta_path) %>%
  mutate(fasta_dest=sub('assemblies/','all_brad_no_pESI_comp/',fasta_path))
...
tr <- read.tree('RAxML_bestTree.all_brad_no_pESI_comp') 
ggtr <- ggtree(tr) 
ggtr <- ggtr %<+% tmp
...
p1 <- 
  ...
  coord_cartesian(clip='off', xlim = c(0,.00018), ylim=c(-2,25)) 
...
p3 <- gheatmap(p = p2, data = AMR_df, offset=.000052, width=.55,
               colnames_angle=90,
               colnames_position = 'top',
               colnames_offset_y = .00015,
               font.size = 2.5, hjust = 0, legend_title='presence' )
...
p4 <- gheatmap(p = p3, data = vir_df, offset=.000089, width=.25,
               colnames_angle=90,
               colnames_position = 'top',
               colnames_offset_y = .00015,
